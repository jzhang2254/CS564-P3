...
void test4()
{
  // testing on scanning negative number
	std::cout << "---------------------" << std::endl;
	std::cout << "Custome Test 1" << std::endl;
	createRelationForward();
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
  try{
    checkPassFail(intScan(&index,-500,GTE,-200,LTE), 0);
    checkPassFail(intScan(&index,-500,GTE,2000,LTE), 2001);
...
void test5()
{
  // testing large relation size (200000 tuples)
	std::cout << "---------------------" << std::endl;
	std::cout << "Custome Test 2: relations of large size" << std::endl;
  test5Helper();
  try{
    BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
  }catch(std::exception &e){
...
void test6()
{
  // testing on inserting into root (leaf node)
	std::cout << "---------------------" << std::endl;
	std::cout << "Custome Test 3: insert into root" << std::endl;
	test6Helper();
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
  checkPassFail(intScan(&index, 0,GTE,4,LTE), 5);
  checkPassFail(intScan(&index,-500,GTE,2000,LTE), 5);
